    ...
    public partial class Login : Gtk.Window
	{
		...
		public Login() : base(Gtk.WindowType.Toplevel)
		{
			try
			{
				...
				this.Title = "Huggle " + System.Windows.Forms.Application.ProductVersion.ToString() + " " + RevisionProvider.GetHash();
	            if (Configuration.Debugging.DevelopmentWarnings)
	            {
	                this.Title = this.Title  + " [devs] - target: " + Core.TargetBuild();
	            }
	            else if (Configuration.Debugging.IsBeta)
	            {
				...
				// load projects
				foreach (string project in Configuration.SiteInfo.Projects.Keys)
				{
				...
				int dl = 0;
				int curr = 0;
				foreach (string language in Configuration.LanguageData.Languages)
				{
					// we need to get the current language id
					if (language == Configuration.LanguageData.Language)
					{
						dl = curr;
					}
					curr++;
		...
		private void login()
		{
			try
			{
				...
				LoggingIn = true;
				Configuration.LoginData.UsingSSL = checkbutton1.Active;
				Configuration.LoginData.Project = combobox1.ActiveText;
				...
				Configuration.LoginData.Username = entry1.Text; // set username
				Configuration.LoginData.Password = entry2.Text; // set password
